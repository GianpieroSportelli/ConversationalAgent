 ...
 * @author SPORT
 */
public class AddingPlan implements ActionPlan {
    ...
    public String getName() {
        return (new JSONObject().accumulate("class", AddingPlan.class.getName())).toString();
    }
    ...
    public List<JSONObject> execute(JSONObject sem, SemanticNet net, Config conf, int epoch, String id_user) {
        ...
        if (qt != null) {
                ...
                if (prop_name.equals("bill")) {
                    if (prop.has("number")) {
                        ...
                        try {
                            ...
                            error.put(Vocabulary.MESSAGE, "errore interno, non riesco a trovare il file: AddingPlan");
                            result.add(error);
                            Logger.getLogger(RegisterPlan.class.getName()).log(Level.SEVERE, null, nf);
                        } catch (IOException ex) {
                            error.put(Vocabulary.MESSAGE, "errore interno, Input-Output: AddingPlan");
                            result.add(error);
                            Logger.getLogger(RegisterPlan.class.getName()).log(Level.SEVERE, null, ex);
                        } catch (ClassNotFoundException ex) {
                            error.put(Vocabulary.MESSAGE, "errore interno, Non trovo la classe Managment: AddingPlan");
                            result.add(error);
                            Logger.getLogger(RegisterPlan.class.getName()).log(Level.SEVERE, null, ex);
                        }
            ...
            response.accumulate(Vocabulary.MESSAGE, "quanto hai avuto?");
            result.add(response);
            ...
            if (sa.getInt("epoch") < epoch) {
                if (sa.getString("name").equals("quantifies")) {
                    try {
                        String url = conf.PATH_CONTABILITA+"-"+id_user+"."+conf.CONTABILITA_EXT;
                        managment = new Management().read(url);
                        if (managment.accrediti > 0) {
                            double amount_d = managment.accrediti;
                            ...
                            domain.accumulate("name", "gain");
                            domain.put("property", bill);
                            JSONObject qtGain = new JSONObject().accumulate("category", "dialog").accumulate("name", "qtRegGain");
                            qtGain.accumulate("domain", domain);
                            result = new ArrayList<>();
                            result.add(qtGain);
                        } else {
                            JSONObject noGain = new JSONObject().accumulate("category", "dialog").accumulate("name", "regNoGain");
                            result = new ArrayList<>();
                            result.add(noGain);
                        }
                        ...
                        result = new ArrayList<>();
                        error.put(Vocabulary.MESSAGE, "non ho trovato il file: AddingPlan");
                        result.add(error);
